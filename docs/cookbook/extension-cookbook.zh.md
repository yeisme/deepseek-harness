...
| Ordo Agent Ops 面板 | tenant-bound host service + snapshot/event cursor + reviewed `dsh.client` 面板；单项 action 使用 ToolView，完整运维视图跳转 Workbench（指南随插件迁至 `agent/harness-plugins`） |
