...
- `ctx.tools.sources(scope?: ScopeKey): ReadonlyMap<string, ToolOrigin>` Where each visible tool came from — `'global'` (the context-global layer), `'scoped'` (some scope's own or inherited layer), or `'transport'` (the reserved Code Mode `run_code`, present only for scopes presenting a code mode). Keyed exactly like `schemas(scope)`, so a reader composing the two answers one composition fact: what the scope sees and which layer supplied it.
