...
- `ctx.systemPrompt.sectionSources(scope?: ScopeKey): ReadonlyMap<string, SectionOrigin>` Where each effective section came from — `'global'` or `'scoped'`, with the nearest supplying layer winning (the same nearest-wins rule as the assembly's shadow merge). Keyed exactly like the section names the scope's assembly resolves; a host reader with no agent passes a standing scope key (an agent preset's mount), the same addressing cold transcript reads use.
