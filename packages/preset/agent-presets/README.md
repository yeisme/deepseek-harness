...
- `ctx.agentPresets.standingFactsFor(id?): Promise<PresetStandingFacts>` The same mount's read-side identity — scope key, the composition file stamp of the mounted generation, and the generation number (how many mounts of this id this process has made) — for callers that prove a mount rather than only address registry views. One derivation behind `standingKeyFor`, so the two can never disagree.
