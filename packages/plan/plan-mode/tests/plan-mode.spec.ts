...
describe('/plan', () => {
  ...
  it('registers /plan-readonly only when permission-presets is composed, and switches both knobs', async () => {
    ...
    expect(bare.commands.list(bareAgent).map(command => command.name)).toEqual(['plan'])
    ...
    openTurn(agent.session)
    expect(ctx.commands.list(agent).map(command => command.name)).toEqual(['plan', 'plan-readonly'])
