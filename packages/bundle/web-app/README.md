...
## Host plugin HMR (opt-in)

Host-side plugin reload is disabled by default for the Web surface. To develop a file-based host plugin without restarting `dsh web`, start with:

```sh
DSH_WEB_HMR=1 DSH_PLUGIN_DEV_ROOT=/absolute/path/to/plugin dsh web
```

`DSH_WEB_HMR=1` enables the shared Cordis HMR row; `DSH_PLUGIN_DEV_ROOT` points at the plugin checkout to watch (use the absolute real path, not the `node_modules` symlink). Editing the plugin's loaded files then hot-reloads that plugin fiber in place, preserving sessions. Without `DSH_PLUGIN_DEV_ROOT`, only the profile directory is watched (config hot reload). This is opt-in because the full reload lifecycle is not yet covered by the published Web e2e suite.
